<template>
	<view class="page_barter diy_detail diy_detail--new" id="barter_details">
		<view class="warp">
			<view class="container-fluid">
				<view class="row">
					...
					<view v-if="$check_field('get','required_barter')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>所需易物</span>
							</view>
					...
					<view v-if="$check_field('get','item_introduction')" class="col-12 col-6">
						<view class="view">
							<view class="diy_title">
								<span>物品介绍</span>
							</view>
...
<!--						<button type="button" class="btn btn-primary" @click="cancel()">返回</button>-->
						...
						<navigator class="btn link"
							:url="'/pages/comment/edit?source_table=barter&source_field=barter_id&source_id=' + obj['barter_id']">
							评论
						</navigator>
		...
		methods: {
			...
			get_comment(obj) {
				this.$get("~/api/comment/get_list?", {
					source_table: "barter",
					source_field: "barter_id",
					source_id: obj["barter_id"]
				}, (res) => {
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/barter/set?barter_id=' + obj["barter_id"], {
					hits
				}, (res) => {
					if(res.result){
