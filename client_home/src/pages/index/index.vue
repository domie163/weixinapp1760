<template>
  <view id="home" class="home--new">
    ...
    <Card
      ...
      :list="list_article"
      v-if="$check_action('/barter/list', 'get')"
    >
	  <list_barter :list="list_barter"></list_barter>
    ...
    <Card
      class="list_diy"
      title="二手物品推荐"
      url="/pages/second_hand_goods/list"
      :list="list_article"
      v-if="$check_action('/second_hand_goods/list', 'get')"
    >
	  <list_second_hand_goods :list="list_second_hand_goods"></list_second_hand_goods>
...
<script>
					...
					import list_barter from "@/components/diy/list_barter.vue";
				import list_second_hand_goods from "@/components/diy/list_second_hand_goods.vue";
  ...
  methods: {
    ...
    sendChat() {
      ...
      this.sendValue = "";
		...
		*/
		get_barter() {
			let url = "~/api/barter/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("以物换物" ,json.result.list);
					this.list_barter = json.result.list;
									}
			});
		},
    				/**
		*  获取二手物品
		*/
		get_second_hand_goods() {
			let url = "~/api/second_hand_goods/get_list?";
									
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("二手物品" ,json.result.list);
					this.list_second_hand_goods = json.result.list;
											this.list_second_hand_goods.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_second_hand_goods ,"second_hand_goods" ,"29849");
									}
			});
		},
