<template>
  <view class="page_lost_and_found_square diy_table diy_table_tml" id="lost_and_found_square_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                    <uni-forms-item label="物品名称" name="item_name">
                      <uni-easyinput type="text" v-model="query.item_name" placeholder="物品名称" />
                    </uni-forms-item>
                    <uni-forms-item label="物品类型" name="item_type">
                  <uni-data-select placeholder="请选择"
            v-model="query['item_type']"
            :localdata="list_item_type"
        ></uni-data-select>
                         </uni-forms-item>
                            <uni-forms-item label="开启悬赏" name="open_the_reward_offer">
                 <uni-data-select placeholder="请选择"
                        v-model="query['open_the_reward_offer']"
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/lost_and_found_square/view?" v-show="user_group == '管理员' || $check_action('/lost_and_found_square/table','add') || $check_action('/lost_and_found_square/view','add')">添加</navigator>
	...
	<view class="warp" >
		<view class="container">
			<view v-for="(o,i) in list" class="row box_style">
									<view class="col" v-if="0 && $check_field('get','publisher')">
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									{{ get_user_publisher(o['publisher']) }}
								</span>
								</view>
													</view>
					</view>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','item_type')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>物品类型</span>
							</view>
						...
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>物品图片</span>
									...
									<view class="col" v-if="0 && $check_field('get','lost_time')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>丢失时间</span>
							</view>
								...
								<span>
									{{ $toTime(o["lost_time"] ,"yyyy-MM-dd hh:mm:ss") }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="0 && $check_field('get','lost_location')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>丢失地点</span>
							</view>
						...
						<view class="view">
							<view class="diy_title" v-if="true">
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','open_the_reward_offer')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>开启悬赏</span>
							</view>
															<view class="diy_field diy_text text">
								<span>
									{{ o["open_the_reward_offer"] }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="0 && $check_field('get','reward_price')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>悬赏价格</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','item_description')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>物品描述</span>
							</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/lost_and_found_square/table','del') || $check_action('/lost_and_found_square/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/lost_and_found_square/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/lost_and_found_square/table','set') || $check_action('/lost_and_found_square/view','set') || $check_action('/lost_and_found_square/view','get')" class="btn btn-primary">详情</navigator>
					                    									</view>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/lost_and_found_square/get_list?like=0",
				url_del: "~/api/lost_and_found_square/del?",
                ...
                list_user_publisher: [],
                                // 物品类型列表
                list_item_type: [{value:"",text:"全部"}],
                                                            // 开启悬赏列表
                list_open_the_reward_offer: [{value:"",text:"全部"}],
                        			}
		},
		methods: {
          search() {
          ...
          reset: function reset() {
            ...
            if(user_group != "管理员"){
                  let sqlwhere = "(";
                              if(user_group=="注册用户"){
                  sqlwhere+= "`publisher` = " + this.user.user_id + " or ";
              }
