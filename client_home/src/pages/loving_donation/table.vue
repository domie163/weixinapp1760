<template>
  <view class="page_loving_donation diy_table diy_table_tml" id="loving_donation_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                    <uni-forms-item label="捐助日期" name="date_of_donation">
                      <uni-easyinput type="text" v-model="query.date_of_donation" placeholder="捐助日期" />
                    </uni-forms-item>
                      </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/loving_donation/view?" v-show="user_group == '管理员' || $check_action('/loving_donation/table','add') || $check_action('/loving_donation/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="1 && $check_field('get','donors')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>捐助人员</span>
								...
								<text>
									{{ get_user_donors(o['donors']) }}
								</text>
							</view>
										</view>
					</view>
													<view v-if="1 && $check_field('get','date_of_donation')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>捐助日期</span>
                          </view>
								...
								<text>
									{{ $toTime(o["date_of_donation"],"yyyy-MM-dd") }}
								</text>
							</view>
										</view>
					</view>
													<view v-if="1 && $check_field('get','donation_items')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>捐助物品</span>
                          </view>
								...
								<text>
									{{ o["donation_items"] }}
													...
													<view v-if="1 && $check_field('get','number_of_donations')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>捐助数量</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/loving_donation/table','del') || $check_action('/loving_donation/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/loving_donation/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/loving_donation/table','set') || $check_action('/loving_donation/view','set') || $check_action('/loving_donation/view','get')" class="btn btn-primary">详情</navigator>
	        					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/loving_donation/get_list?like=0",
				url_del: "~/api/loving_donation/del?",
		...
		methods: {
          ...
          reset: function reset() {
            ...
            if(user_group != "管理员"){
                  let sqlwhere = "(";
                              if(user_group=="注册用户"){
                  sqlwhere+= "`donors` = " + this.user.user_id + " or ";
              }
