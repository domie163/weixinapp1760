          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','user_name') || $check_field('add','user_name') || $check_field('set','user_name')" label="用户姓名" name="user_name">
                            <uni-easyinput type="text" v-model="form['user_name']" v-if="user_group === '管理员' || (form['registered_users_id'] && $check_field('set','user_name')) || (!form['registered_users_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','user_gender') || $check_field('add','user_gender') || $check_field('set','user_gender')" label="用户性别" name="user_gender">
                  ...
                  v-if="user_group === '管理员' || (form['registered_users_id'] && $check_field('set','user_gender')) || (!form['registered_users_id'] && $check_field('add','user_gender'))"
                ></uni-data-select>
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','user_identity') || $check_field('add','user_identity') || $check_field('set','user_identity')" label="用户身份" name="user_identity">
                    <uni-data-select
                  v-model="form.user_identity"
                  :localdata="list_user_identity"
                  :clear="!disabledObj['user_identity_isDisabled']"
                  :disabled="disabledObj['user_identity_isDisabled']"
                  v-if="user_group === '管理员' || (form['registered_users_id'] && $check_field('set','user_identity')) || (!form['registered_users_id'] && $check_field('add','user_identity'))"
                ></uni-data-select>
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','user_identity')">
                  {{ form['user_identity'] }}
                </text>
                  </uni-forms-item>
...
export default {
      ...
      form: {
          ...
          disabledObj:{
                        "user_name_isDisabled": false,
                                ...
                                "user_identity_isDisabled": false,
                                  },
                                          // 用户性别选项列表
          list_user_gender: [],
                                      // 用户身份选项列表
  ...
  methods: {
     ...
     * 获取用户身份列表
     */
    async get_list_user_identity() {
          ['学生','教职工'].map((o) => this.list_user_identity.push({value:o,text:o}));
            },
