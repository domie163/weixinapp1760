<template>
	<view class="page_second_hand_goods diy_list diy_list_page div_list_tml" id="second_hand_goods_list">
		<!-- 筛选模块(开始) -->
		<view>
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?second_hand_goods_id=' + o['second_hand_goods_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=second_hand_goods", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["second_hand_goods_id"] === o["source_id"]){
