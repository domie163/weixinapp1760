<template>
  <view class="page_second_hand_goods diy_table diy_table_tml" id="second_hand_goods_table">
    <!-- 筛选模块(开始) -->
    <view>
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/second_hand_goods/view?" v-show="user_group == '管理员' || $check_action('/second_hand_goods/table','add') || $check_action('/second_hand_goods/view','add')">添加</navigator>
	...
	<view class="warp" >
		<view class="container">
			<view v-for="(o,i) in list" class="row box_style">
									<view class="col" v-if="0 && $check_field('get','serial_number')">
						<view class="view">
						...
						<view class="view">
							<view class="diy_title" v-if=" false">
								<span>封面图片</span>
									...
									<view class="col" v-if="0 && $check_field('get','item_quantity')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>物品数量</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="1 && $check_field('get','unit_price')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>单件价格</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','commodity_specification')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>商品规格</span>
							</view>
								...
								<span>
									...
									<view class="col" v-if="0 && $check_field('get','introduction_details')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>介绍详情</span>
							</view>
															<view class="diy_field diy_text text">
								<span>
									{{ o["introduction_details"] }}
								</span>
								</view>
													</view>
					</view>
									<view class="col" v-if="0 && $check_field('get','selling_users')">
						<view class="view">
							<view class="diy_title" v-if="true">
								<span>售卖用户</span>
							</view>
								...
								<span>
									{{ get_user_selling_users(o['selling_users']) }}
								</span>
								</view>
													</view>
					</view>
				...
				<view class="col">
					<view class="view">
						<view class="create_time_block diy_filed diy_time">
                          ...
                          <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/second_hand_goods/table','del') || $check_action('/second_hand_goods/view','del')">删除</button>
                        </view>
					</view>
				</view>
				<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/second_hand_goods/view?' + field + '=' + o[field]"
						v-show="user_group == '管理员' || $check_action('/second_hand_goods/table','set') || $check_action('/second_hand_goods/view','set') || $check_action('/second_hand_goods/view','get')" class="btn btn-primary">详情</navigator>
					                    																		<!--跨表按钮-->
															<a type="button" size="mini" class="btn btn-primary" v-if="$check_action('/purchase_order/view','add')" @click="to_form('/pages/purchase_order/view',o)">
									<span>购买</span>
								</a>
																						</view>
			</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/second_hand_goods/get_list?like=0",
				url_del: "~/api/second_hand_goods/del?",
		...
		methods: {
          ...
          reset: function reset() {
            ...
            if(user_group != "管理员"){
                  let sqlwhere = "(";
                                                                                                                                                              if(user_group=="注册用户"){
                  sqlwhere+= "`selling_users` = " + this.user.user_id + " or ";
              }
